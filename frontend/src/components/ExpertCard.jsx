...
const ExpertCard = ({ expert, onRequestMeeting, onToggleFollow, onViewProfile, isFollowing = false, userType = 'patient' }) => {
  ...
  return (
    <div className="card-hover group animate-fade-in-up">
      ...
      <div className="flex items-start mb-5">
        ...
        {expert.profile_picture ? (
          <img
            ...
            alt={expert.name}
            className="w-20 h-20 rounded-2xl object-cover mr-4 shadow-duo group-hover:scale-110 transition-transform duration-300 flex-shrink-0"
          />
        ) : (
      ...
      <div className="flex gap-2.5">
        ...
        <button
          onClick={() => onViewProfile && onViewProfile(expert)}
          className="flex-1 py-3 px-4 border-2 border-gray-300 hover:border-primary-500 rounded-xl font-bold text-gray-700 hover:text-primary-600 transition-all duration-200 hover:scale-105 active:scale-95 text-sm"
        >
          View Profile
        </button>
      </div>
